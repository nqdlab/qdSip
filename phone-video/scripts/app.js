    ...
    ctxSip = {
        ...
        newSession : function(newSess) {
			...
			newSess.on('trackAdded', function() {
				// Declare a RTCPeerConnection			  
				var pc = newSess.sessionDescriptionHandler.peerConnection;			  
				// Gets remote tracks. The getReceivers method is a webrtc native method.
				...
				//To  get rid of the issue: caught (in promise) DOMException: The play() request was interrupted by a new load request.
				...
				setTimeout(() => {
					// Gets local tracks. The getSenders method is a webrtc native method.
					var localStream = new MediaStream();
					pc.getSenders().forEach(function(sender) {
						//only require tracks in each sender 
						if(sender.track){
					...
					var playPromiseLocal = localVideo.play();
					
					//To  get rid of the issue: caught (in promise) DOMException: The play() request was interrupted by a new load request.
					if (playPromiseLocal !== undefined) {
						playPromiseLocal.then(_ => {
						  // Automatic playback started!
						  // Show playing UI.
						}).catch(error => {
						  // Auto-play was prevented
        ...
        sipCall : function(target) {
            ...
            try {
                var outgoingSession = ctxSip.phone.invite(target, {
                    sessionDescriptionHandlerOptions : {
                        constraints : { audio : true, video : true }               
                    }
                });
                outgoingSession.direction = 'outgoing';
                ctxSip.newSession(outgoingSession);		  
